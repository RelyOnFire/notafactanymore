---
claim: "Migraine is caused simply by blood vessels constricting and then dilating."
currentUnderstanding: "Migraine is a neurological disorder involving brain networks, sensory processing, the trigeminal nerve system, signaling molecules, and blood vessels. Changes in vessel size can occur during an attack, but dilation by itself is neither necessary nor enough to explain migraine."
whyItChanged: "The older vascular theory linked aura to narrowing of cranial vessels and headache to their later dilation. Brain imaging, blood-flow studies, and treatments aimed at neural signaling showed that attacks can begin without that simple vessel sequence and that migraine involves broader nervous-system changes."
status: "Superseded"
category: "Neuroscience"
acceptedApproximately: "1940s–1990s"
acceptedYear: 1945
changedApproximately: "1980s–present"
timelineYear: 1990
summary: "Migraine changed from a one-cause blood-vessel model to a broader neurological and neurovascular disorder."
featured: false
publishedAt: "2026-07-25"
reviewedAt: "2026-08-20"
sources:
  - title: "Pathophysiology of Migraine"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC5539409/"
    publisher: "PubMed Central"
    purpose: "Previous belief"
    note: "Explains how Wolff's work gave the vascular theory momentum and made it the leading account of migraine."
  - title: "Exciting excitable brains"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC3611672/"
    publisher: "PubMed Central"
    purpose: "Historical context"
    note: "States that the vascular theory was superseded by evidence for primary brain and neurovascular mechanisms."
  - title: "Vascular Contributions to Migraine"
    url: "https://pmc.ncbi.nlm.nih.gov/articles/PMC6088188/"
    publisher: "PubMed Central"
    purpose: "Current evidence"
    note: "Reviews the modern view that vascular changes may contribute but do not alone explain migraine."
---

For much of the twentieth century, migraine was explained mainly as a problem of blood vessels in and around the brain.

The classic sequence was simple: vessels first constricted, or narrowed, producing the visual and sensory symptoms called aura. They then dilated, or widened, and that expansion was supposed to cause the throbbing headache.

That model fit some observations, but not all of them.

Brain imaging and blood-flow studies showed that migraine attacks can begin without the predicted sequence of vessel narrowing and widening. Changes in blood-vessel size also do not track symptoms consistently enough to explain the whole disorder.

Research increasingly pointed to the nervous system itself. The trigeminal system—a major sensory pathway carrying signals from the face, head, and the coverings of the brain—becomes involved during attacks. Brain networks that regulate pain and sensory processing also change, and signaling molecules such as CGRP help transmit and amplify migraine-related signals.

Treatments that block CGRP or other neural pathways can prevent or relieve migraine without working as simple mechanical vasoconstrictors.

Blood vessels still matter, and migraine is often described as neurovascular because nerve and vessel processes interact. What disappeared was the idea that a migraine is simply a headache caused by an artery becoming wider.

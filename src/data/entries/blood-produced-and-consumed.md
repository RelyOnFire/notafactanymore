---
claim: Blood is continually produced in the liver, flows outward to the body's tissues, and is consumed rather than recirculated.
currentUnderstanding: The heart pumps blood through a closed circulatory system. Blood repeatedly travels through arteries, capillaries, and veins, carrying gases, nutrients, hormones, wastes, and immune cells.
whyItChanged: William Harvey used anatomical observations, valve direction, experiments, and quantitative estimates of cardiac output to show that the volume of blood passing through the heart was far too large to be continuously produced and consumed.
status: Superseded
category: Medicine
acceptedApproximately: 2nd century through the early 17th century
acceptedYear: 150
changedApproximately: 1620s–1660s
timelineYear: 1628
lifespanComparable: true
summary: Galen's outward-flow model of blood was replaced by the discovery that blood continuously circulates through the heart and vessels.
featured: false
publishedAt: '2026-07-17'
reviewedAt: '2026-08-13'
media:
  - id: "blood-produced-and-consumed-explainer"
    type: "image"
    src: "/media/entries/blood-produced-and-consumed/blood-produced-and-consumed-explainer.en.svg"
    width: 760
    height: 1710
    alt: "Comparison diagram. The first section shows blood produced in the liver and flowing outward toward tissues where it is consumed. The second shows a closed loop in which the heart drives blood through arteries and back through veins, with pulmonary circulation included."
    caption: "A simplified comparison of the Galenic blood model and Harvey’s circulation. In the older framework, blood was continually produced and used by the tissues; Harvey’s experiments and quantitative reasoning instead supported repeated circulation driven by the heart."
    credit: "Original graphic: Not a Fact Anymore"
    sourceLabel: "Not a Fact Anymore"
    sourceUrl: "https://notafactanymore.com/entries/blood-produced-and-consumed/"
    rights: "original"
    layout: "inline"
sources:
- title: 'Discovery of the cardiovascular system: from Galen to William Harvey'
  url: https://pubmed.ncbi.nlm.nih.gov/21781247/
  publisher: PubMed
  purpose: Historical context
  note: Summarizes Galen's claim that the liver produced blood that was distributed outward and notes that the model held sway for roughly fifteen centuries.
- title: 'De Motu Cordis: the Lumleian Lecture of 1616'
  url: https://pmc.ncbi.nlm.nih.gov/articles/PMC1847732/
  publisher: PubMed Central
  purpose: Previous belief
  note: Describes the Galenic model in which food became blood in the liver and was then distributed to the body.
- title: Lessons from the life of William Harvey
  url: https://pmc.ncbi.nlm.nih.gov/articles/PMC10566385/
  publisher: PubMed Central
  purpose: Current evidence
  note: Explains Harvey's discovery that blood flows in a circle and his 1628 publication of the finding.
- title: The history of the theory of the circulation of the blood
  url: https://pubmed.ncbi.nlm.nih.gov/27791994/
  publisher: PubMed
  purpose: Historical context
  note: Reviews the development of the idea that blood repeatedly circulates through the cardiovascular system.
- title: 'Discovery of the cardiovascular system: from Galen to William Harvey'
  url: https://onlinelibrary.wiley.com/doi/full/10.1111/j.1538-7836.2011.04312.x
  publisher: Journal of Thrombosis and Haemostasis
  purpose: Historical context
  note: Detailed review of Galen's liver-produced, outward-flowing blood model and its roughly fifteen-century dominance before Harvey.
---

Galen's circulatory model was sophisticated enough to explain a great deal of anatomy without containing circulation in the modern sense. In the second century CE, Galen described two linked but distinct vascular systems. Food was transformed into blood in the liver and sent outward through the veins to nourish tissues. Arteries carried blood mixed with *pneuma*, a life-associated principle linked to respiration. Blood could pass between the venous and arterial sides through pathways Galen believed existed, including invisible pores in the heart's septum.

The system was open-ended: tissues consumed the blood delivered to them, so the liver continually made more. That is why Harvey's later arithmetic was so damaging. Once he estimated how much blood the heart expelled with each beat, the implied daily volume was far too large for the liver to manufacture and the tissues to consume continuously.

Galen's medical writings became extraordinarily authoritative across later Byzantine, Islamic, and European medicine, but the history was never a simple 1,500-year freeze. Ibn al-Nafis described pulmonary circulation in the 13th century and rejected Galen's invisible septal pores. Renaissance anatomists such as Vesalius corrected additional Galenic anatomy before Harvey.

Harvey's 1628 *De Motu Cordis* united anatomy, valve direction, experiment, and quantitative reasoning into the closed-circuit model. Marcello Malpighi's later microscopic observations of capillaries supplied a connection Harvey had inferred but could not directly see.

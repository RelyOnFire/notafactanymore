---
claim: Heavenly bodies are made from a special incorruptible substance—aether or quintessence—rather than the matter found on Earth.
currentUnderstanding: The Sun, stars, planets, moons, asteroids, and comets are made from the same chemical elements and governed by the same fundamental physics studied on Earth, although their compositions and physical states differ.
whyItChanged: Early-modern astronomy and mechanics eroded the physical boundary between heaven and Earth. Spectroscopy later showed directly that celestial bodies contain familiar chemical elements.
status: Superseded
category: Astronomy
acceptedApproximately: Aristotelian cosmology from the 4th century BCE through the Renaissance
acceptedYear: -350
changedApproximately: 17th–19th centuries
timelineYear: 1687
lifespanComparable: false
summary: Aristotle gave the heavens a special incorruptible substance, aether; modern astronomy finds the same elements and physical laws throughout the cosmos.
featured: false
publishedAt: '2026-08-13'
reviewedAt: '2026-08-13'
sources:
- title: Natural Philosophy in the Renaissance
  url: https://plato.stanford.edu/entries/natphil-ren/
  publisher: Stanford Encyclopedia of Philosophy
  purpose: Previous belief
  note: Describes the Aristotelian heavens as perfect, immutable, and composed of the single element aether.
- title: Newton's Principia
  url: https://www.newtonproject.ox.ac.uk/view/texts/normalized/NATP00089
  publisher: The Newton Project, University of Oxford
  purpose: Historical context
  note: Represents the early-modern unification of terrestrial and celestial mechanics under common mathematical laws.
- title: Composition of Stars
  url: https://asd.gsfc.nasa.gov/Koji.Mukai/spectra/index.html
  publisher: NASA Goddard Space Flight Center
  purpose: Current evidence
  note: Shows how stellar spectra reveal familiar chemical elements in stars.
---

Aether was more than a mysterious extra material. It solved a structural problem in Aristotelian physics.

Earthly matter was built from earth, water, air, and fire. Those elements had natural places, could transform into one another, and belonged to a world of generation and decay. But the heavens seemed to move endlessly in perfect circles without wearing out. Aristotle therefore assigned them a different kind of matter: **aether**, whose natural motion was eternal circular motion.

The result was a two-material universe. Rocks, oceans, flames, and bodies were one kind of nature; stars and planets were another.

The distinction weakened as early-modern astronomy increasingly treated celestial objects as physical bodies rather than a separate ontological class. Newton's mechanics was especially important because one gravitational framework could describe a falling body on Earth and the Moon's orbit. In the nineteenth century, spectroscopy supplied a direct chemical test: stellar spectra reveal familiar elements such as hydrogen, helium, sodium, calcium, and iron.

There is no single clean date when “aether” ceased being accepted, so this card should not be used for precise lifespan statistics. What changed was a centuries-long collapse of the idea that celestial matter belongs to a fundamentally separate category.
